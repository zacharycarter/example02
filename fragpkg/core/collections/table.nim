...
proc `whenJobsFinished=`*[S,T](self: var Table[S,T], newVal: T) {.inline.}=
  `value=`(self.allJobsFinishedCallback, cast[pointer](newVal))

proc whenJobsFinished*[S,T](self: var Table[S,T]): T {.inline.}=
  cast[T](self.allJobsFinishedCallback.value)

iterator items*[S: static[int],T](self: var Table[S,T]): T =
  if self.elements.value != 0:
    for i in 0 .. self.elements.value.int - 1:
      yield cast[T](self.data[i].value)

proc init*[S: static[int] ,T](self: var Table[S, T]) =
  `value=`(self.elements, 0)
  #self.elements.value = 0

proc append*[S: static[int], T](self: var Table[S, T], newData: T) =
...
proc remove*[S: static[int], T](self: var Table[S, T], p: T) =
  var i = 0
...
proc len*[S: static[int], T](self: var Table[S,T]): uint16 =
